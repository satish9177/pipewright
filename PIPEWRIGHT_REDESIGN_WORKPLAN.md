...
12. ~~**Next: human review of item 11** (per-PR gate — the highest-risk hunk of Phase 2: the T2 rollback move out of `tester.py`).~~ **Item 11 reviewed & accepted 2026-06-11** (verified: `chunk_driver.run_tests_with_rollback` same-thread/post-rollback report, no double rollback, resume fail-closed preserved, §11.2a apply-phase no-auto-retry golden, and the endorsed `_execute_retry_attempt` verify-step reroute). **Next: item 12 (human_retry collapse + attempt ledger + P7) — do not start it unreviewed.**
