...
- **Phase 3 — needs Phase 2 (done) + §5.3 (DECIDED 2026-06-12, conservative variant):** continuation / steer turns.
  ...
  - **Item 14** (post-success refinement: steer a completed chunk → new commit + cumulative final diff) **deferred one slice by decision** — becomes the next brief **after item 13's human review**; it gates on nothing further (§5.3 is decided).
...
15. **Next: human review of item 13** (the per-PR gate — this is the first inbound free-text execution path; review the §5.3 gate, the shared-budget accounting, and the turn-log discipline). After acceptance: repurpose `PIPEWRIGHT_REDESIGN_IMPL_BRIEF.md` for **item 14** (post-success refinement + cumulative final diff — gates on nothing further). **Phase 4 still needs the §5.4 decision.**
