...
**Date:** 2026-06-12
**What this file is:** the **rolling** implementation handoff brief for the redesign. Each phase's active spec lives here; when a slice lands, this file is repurposed for the next one. **DORMANT as of 2026-06-12: item 17 — the LAST Phase-4 slice and the last Area-A Pass-1 slice — has fully landed. 17a (trivial-task stage profile) merged via PR #288 (`221cc9c` / `24e24b8`); 17b (provider prompt caching) merged via PR #290 (`e2b7cc9`). Do NOT re-review or re-implement either — outcomes are recorded in `PIPEWRIGHT_REDESIGN_WORKPLAN.md`. This brief has no active occupant and stays dormant until the next pass (e.g. Area B / Pass 2 memory work) repurposes it. The content below is the item-17 design brief, retained for historical reference only.** The previous occupant (item 16 — phase/narrative read-model extending `operator_state`) landed on `develop` (`c7bfbf4` / `d3deb65`), reviewed before push. Its outcome is recorded in `PIPEWRIGHT_REDESIGN_WORKPLAN.md`.
**For:** historical reference only — Phase 4 item 17 (17a + 17b) has landed; nothing in this brief is active work.
...
> **Status: COMPLETE. 17a merged via PR #288; 17b merged via PR #290 (2026-06-12).** Decisions E0–E5 were ratified 2026-06-12 and the as-built design lives in `PIPEWRIGHT_ITEM17_DESIGN.md`. **Item 17 is fully landed — do not re-implement either half.** With it, **Phase 4 and Area A (Pipeline) Pass 1 are COMPLETE.** This brief is dormant; the spec below is historical. Outstanding follow-ups are operational only: monitor the 17a soak (`docs/design/trivial-task-profile-soak.md`) and flip `PROMPT_CACHE_ENABLED = True` to activate 17b caching for Anthropic-routed roles.
...
**Status (2026-06-12): ALL DONE.** 17a (PR #288) and 17b (PR #290) have landed; Phase 4 and Area A Pass 1 are COMPLETE.

1. ~~**`PIPEWRIGHT_REDESIGN_WORKPLAN.md`** — mark item 17 done in the Phase 4 sequence + "How to resume" + the TL;DR. **Note that Phase 4 — and Area A Pass 1 — is COMPLETE.**~~ ✅ **done** (TL;DR, doc table, status override, Phase 4 sequence, item-17a/17b implementation notes, and resume steps 21–23 all stamped COMPLETE).
2. ~~**`backend/pipeline/policy.py`** docstring + the new constants — `MERGED_PROFILE_SAMPLE_PCT` + `TRIVIAL_PROFILE_DENYLIST_PATTERNS` (17a) and the cache flags `PROMPT_CACHE_ENABLED` / `GEMINI_EXPLICIT_CACHE_ENABLED` (17b).~~ ✅ **done** — all four constants landed commented in `policy.py`; model selection stays in `role_config.py`.
3. ~~**A short `docs/design/` note (or the proposal §18.3 section)** — the trivial-profile eligibility predicate as-built and the **documented SQL** for the §18.3 soak + the D12 rollback trigger (the measurement is SQL over the ledger, not code).~~ ✅ **done for 17a:** `docs/design/trivial-task-profile-soak.md` (operator-facing — `stage_profile` values, cohort-comparison SQL over `chunk_attempts`, and the `MERGED_PROFILE_SAMPLE_PCT = 0` rollback). A 17b caching-usage view can extend it later if desired.
...
5. ~~**This file** — once item **17b** lands + is reviewed, this rolling brief has **no further Phase-4 occupant**: note that Area A Pass 1 is complete and the brief is dormant until the next pass repurposes it.~~ ✅ **done** — the header + status callout now mark the brief **dormant**; Area A Pass 1 complete.
6. These planning docs are **tracked**. Update content; **do not commit** doc or code changes unless the maintainer asks. If asked to commit: branch off `develop` first (never straight to `develop`/`main`), one item per commit, end with the repo's `Co-Authored-By` trailer.
