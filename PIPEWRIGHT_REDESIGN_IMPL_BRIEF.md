...
**Date:** 2026-06-12
**What this file is:** the **rolling** implementation handoff brief for the redesign. Each phase's active spec lives here; when a slice lands, this file is repurposed for the next one. **Current contents: Phase 4, slice 3 — item 17 (trivial-task stage profile + provider prompt caching), the LAST Phase-4 slice and the last Area-A Pass-1 slice.** The previous occupant (item 16 — phase/narrative read-model extending `operator_state`) landed on `develop` (`c7bfbf4` / `d3deb65`), reviewed before push — do **not** re-review it. Its outcome is recorded in `PIPEWRIGHT_REDESIGN_WORKPLAN.md`.
**For:** the model implementing Phase 4 item 17. Re-verify every `file:line` against the live code before you cite or edit it; items 13–16 moved line numbers and the repo keeps moving.
...
> **Status: DESIGN-FIRST. Not yet implemented.** This document is the design brief for the maintainer to review. **Do not write code until the maintainer ratifies the decisions in §17.0.** After drafting/ratification, the implementer produces a short design doc answering E0–E5, gets it reviewed, then implements.
...
1. **`PIPEWRIGHT_REDESIGN_WORKPLAN.md`** — mark item 17 done in the Phase 4 sequence + "How to resume" + the TL;DR. **Note that Phase 4 — and Area A Pass 1 — is COMPLETE.**
2. **`backend/pipeline/policy.py`** docstring + the new constants — document the profile sampling knob, the E1 denylist (if used), and the cache flag(s); restate that model selection stays in `role_config.py`.
3. **A short `docs/design/` note (or the proposal §18.3 section)** — the trivial-profile eligibility predicate as-built and the **documented SQL** for the §18.3 soak + the D12 rollback trigger (the measurement is SQL over the ledger, not code).
4. **The `chunk_attempts` ledger comment** (`schema.sql:199-204` / `chunk_attempt_store.py` docstring) — record the new `stage_profile` column and its closed enum.
5. **This file** — once item 17 lands + is reviewed, this rolling brief has **no further Phase-4 occupant**: note that Area A Pass 1 is complete and the brief is dormant until the next pass (e.g. Area B / Pass 2 memory work) repurposes it.
6. These planning docs are **untracked**. Update content; **do not commit** them (or any code) unless the maintainer asks. If asked to commit: branch off `develop` first (never straight to `develop`/`main`), one item per commit, end with the repo's `Co-Authored-By` trailer.
