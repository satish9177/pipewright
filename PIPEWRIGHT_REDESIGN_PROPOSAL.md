...
>
> **Reconciled 2026-06-14 (Row 12 PR-A opening).** Row 11 — detection rules-as-data — is now complete and closed (PR-A rules extraction + pre-PR-B fixture hardening + PR-B advisory repo reality signals + PR-C test-command detector refactor, all merged; see `PIPEWRIGHT_REDESIGN_WORKPLAN.md`). The active window below moves on to **§23 order-row 12 — request-aware selection + mandatory tier + adaptive guardrail (Pass 2 §11.1)**, opened **PR-A only** — no-op-by-default scaffolding. E.1/E.2 are updated accordingly; the §23 sequence and §24 roster are unchanged (Row 12 already sits in §23; **D5 governs the later PR-C, not PR-A**); E.3–E.5 remain binding.
>
...
**This build cycle is limited to exactly one slice: §23 order-row 12 PR-A — request-aware selection scaffolding (Pass 2 §11.1), no-op by default.** Maintainer decision 2026-06-14: Row 11 is complete and closed, so open Area B's next memory row — Row 12 — one sub-slice at a time, safest first. **Row 12 PR-A is decision-free**: it introduces a dormant `request_context` (default `None`, byte-identical block), a `security`/`forbidden_paths`-only mandatory safety tier, and policy single-sourcing of the injection budgets / token estimator / adaptive guardrail — with **no relevance omission, no request-aware ordering, no orchestrator plumbing, and no D5 activation**. (§24's gating map lists Row 12 as needing D5; that gate governs the later **PR-C**, not PR-A, which never acts on D5.) Design-first per the established discipline: the human-reviewed design brief in `PIPEWRIGHT_REDESIGN_IMPL_BRIEF.md` precedes any code.

**Row 12 ships in ordered sub-PRs; this cycle opens PR-A only:**

- **PR-A (this cycle) — request-aware selection scaffolding, no-op by default.** In `backend/memory/prompt_builder.py`: an optional `request_context` parameter (default `None`; `None` preserves today's injected block byte-for-byte, golden-locked); a mandatory safety tier for `security` + `forbidden_paths` **only** (structurally un-droppable — never a `budget_dropped` exclusion; loud-fail if the tier alone exceeds the cap); policy single-sourcing of `ROLE_TOKEN_BUDGETS` + the token estimator + the adaptive guardrail into `policy.py` (model/window resolved via `role_config`); and a **defined-but-dormant** `not_relevant_to_request` provenance reason that PR-A must **never emit**. No relevance omission, no ordering change, no orchestrator plumbing, no schema, no D5 activation.
- **PR-B (later, deferred) — relevance *ordering* only.** When a non-`None` `request_context` is supplied, order the relevance tier by a deterministic rung-0 signal (lexical overlap on chunk title / description / `files_expected`); same set injected (no omission); plumbs `request_context` from the orchestrator. Not opened this cycle.
...
**Hard stop after Row 12 PR-A.** Run a real self-use smoke before starting any of: Row 12 **PR-B** (relevance ordering) or **PR-C** (relevance omission / pinning, D5); the early-band backfill set above; the deeper Area B memory rows **16** (post-run hygiene, D7), **19** (retriever + FTS rung 1), **23** (vector rung 2, D6); or the §21 thread UI rows (**22b–22e**, D13). The obsolete original hard-stop language ("before starting the stage driver, execution steering, full thread UI, or vector memory") is void for the engine rows — they shipped — and survives only as "before vector memory / thread UI."

## E.2 Accepted defaults for this cycle

**Row 12 PR-A (request-aware selection scaffolding) is decision-free** — it never exercises D5 (no omission, no pinning, no ordering). §24's gating map lists Row 12 as needing **D5**, but that gate governs the later **PR-C**, not PR-A. No new default needs accepting to start this cycle. **D5 must be confirmed before PR-C** (see the D5 wording tension recorded in `PIPEWRIGHT_REDESIGN_IMPL_BRIEF.md`).
...
Vector memory (row 23), the retriever/FTS rung (row 19), Row 12 **PR-B/PR-C** (relevance ordering / omission), post-run hygiene (row 16), and the thread UI (rows 22b–22e) remain outside this window unless a later maintainer decision explicitly opens them. (Row 12 **PR-A** is the active window above; Row 11 is complete.)
