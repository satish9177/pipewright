...
>
> **Reconciled again 2026-06-14.** The M5 suggestion-quality gate (§23 order-row 7) is now **built and merged** (PR #292), and its post-M5 self-use smoke / closeout is recorded in `docs/testing/memory-m5-suggestion-quality-smoke.md` — satisfying the prior "hard stop after the M5 gate." The active window below moves on to **§23 order-row 11 — detection rules-as-data**, opened as the next memory slice (PR-A first). E.1/E.2 are updated accordingly; the §23 sequence and §24 roster are unchanged (Row 11 already sits in §23 with no new decision); E.3–E.5 remain binding.
>
> **Prior reconciliation 2026-06-14 (Row 12 PR-A opening).** Row 11 — detection rules-as-data — was complete and closed (PR-A rules extraction + pre-PR-B fixture hardening + PR-B advisory repo reality signals + PR-C test-command detector refactor, all merged; see `PIPEWRIGHT_REDESIGN_WORKPLAN.md`). The window moved on to **§23 order-row 12 — request-aware selection + mandatory tier + adaptive guardrail (Pass 2 §11.1)**, opened **PR-A only** — no-op-by-default scaffolding. E.1/E.2 were updated accordingly; the §23 sequence and §24 roster were unchanged (Row 12 already sits in §23; **D5 governs the later PR-C, not PR-A**); E.3–E.5 remained binding.
>
> **Reconciled 2026-06-14 (Row 12 PR-A closeout).** Row 12 PR-A is now complete:
> no-op-by-default request-aware selection scaffolding only. `request_context`
> exists but remains dormant; `request_context=None` preserves existing memory
> injection behavior; memory budgets and the token estimator are single-sourced
> in policy; adaptive budget scaffolding exists but remains disabled;
> `security` / `forbidden_paths` are mandatory safety facts that cannot be
> `budget_dropped`; mandatory overflow uses `MandatoryMemoryBudgetExceeded`;
> prompt-preview maps mandatory overflow to HTTP 422; and
> `not_relevant_to_request` is defined but dormant and not emitted. E.1/E.2 now
> record the pause before Row 12 PR-B. E.3–E.5 remain binding.
>
> **Reconciled 2026-06-14 (Row 12 PR-B closeout).** Row 12 PR-B is now complete:
> request-aware memory **relevance ordering** only. `run_coder` plumbs a
> `RequestContext` (coder-only) into `build_project_memory_block_detailed`; a
> populated context reorders **only** the non-mandatory relevance tier by a
> deterministic rung-0 signal (path-token overlap, then content-token Jaccard,
> reusing the `memory_trust` helpers), tie-broken by the existing sort key.
> `request_context=None` stays byte-identical; all-zero overlap preserves the
> legacy order; the `security`/`forbidden_paths` mandatory tier is never scored,
> reordered, or budget-dropped; and there is **no omission, no
> `not_relevant_to_request` emission, no D5 activation, no pinning, no off-switch,
...
>
> **Reconciled 2026-06-14 (Row 12 PR-C — D5 confirmed; merged dormant-by-default; Row 12 complete).**
...
**Built and merged to `develop` (do not re-open):** §23 order-rows **1** (test-command detection), **2–6** (Phase 0), **8–10** (Signal C, failure-type split + INFRA_ERROR auto-retry, baseline-aware verification), the Phase-2/3/4 engine rows **13–15 / 17–18 / 20–22** (stage extraction + driver + attempt ledger; steered attempts + post-success refinement; reviewer ack gate + narrative read-model + trivial-task profile + prompt caching), and the Phase-2 golden/characterization coverage that stands in for **12b**; the post-Pass-1 red-team hardening (F1–F3, F5); and the first Area B (memory) slice, **order-row 7 — the M5 suggestion-quality gate** (PR #292: deterministic scorer + objective-junk floor + per-run caps + structured coder handoff channel + additive `quality_score` column; closeout recorded in `docs/testing/memory-m5-suggestion-quality-smoke.md`); **order-row 11 — detection rules-as-data** (PR-A rules extraction + pre-PR-B fixture hardening + PR-B advisory repo reality signals for non-DB dimensions + PR-C test-command detector refactor — all behavior-preserving, no memory mutation; see the workplan); **order-row 12 PR-A — request-aware selection scaffolding** (no-op-by-default; no relevance ordering/omission/D5 activation; no schema/frontend/gate/scope/Git/PR/memory mutation/vector/FTS/thread UI change); and **order-row 12 PR-B — request-aware relevance *ordering*** (coder-only `request_context` plumbing; deterministic rung-0 path/token ordering of the non-mandatory tier, reusing the `memory_trust` helpers; `request_context=None` byte-identical; mandatory safety tier never scored/reordered/dropped; ordering only — no omission, no `not_relevant_to_request` emission, no D5 activation, no schema/frontend/gate/scope/Git/PR/memory-mutation change).
...
> **Closeout note (docs-only, 2026-06-18):** order-rows **6b** and **6c** are no longer open — a verification audit confirmed both are **implementation-complete, wired, and test-green** (they had landed but were never recorded as done). They are deterministic advisory-only post-triage backstops in `backend/pipeline/chunk_sizing.py` / `backend/pipeline/chunk_isolation.py`, wired via `backend/pipeline/plan_postprocess.py` on both the initial-plan and plan-turn paths; they append `[SIZE]` / `[ISOLATION]` notes to chunk rationale before persistence and render as non-blocking `ChunkPlanPanel` banners. Both ship **dormant behind default-off flags** (`PIPEWRIGHT_CHUNK_SIZING_ADVISORY_ENABLED` / `PIPEWRIGHT_CHUNK_ISOLATION_ADVISORY_ENABLED`); flag-off is byte-identical passthrough. Row 6c matches **D14 option (a)** as designed in §18.8 (advisory-only; no `requires_human_review` escalation; no hard gate). This does not alter the original §18.7 / §18.8 / §23 / D14 design intent above; it only records that the work shipped. The remaining backfill items (**7a-`plan_versions`**, **7b**, **9b**) are unaffected. See `PIPEWRIGHT_REDESIGN_IMPL_BRIEF.md` (Row 6b / Row 6c closeout) and `docs/status/current-state.md`.
...
**Row 12 ships in ordered sub-PRs; PR-A, PR-B, and PR-C are all merged — Row 12 is complete (PR-C dormant-by-default, D5 confirmed):**
