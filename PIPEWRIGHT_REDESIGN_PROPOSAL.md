...
**This build cycle is limited to exactly one slice: §23 order-row 11 — detection rules-as-data (Pass 2 §11.4).** Maintainer decision 2026-06-14: the M5 gate (row 7) shipped and its post-M5 self-use smoke is recorded, satisfying the prior hard-stop; open Area B's next slice. Row 11 is **decision-free** (§24 gating map: "Items 1–7a (except 6c), **11**, 12b–16b proceed on no open decisions") and is the prerequisite for the decision-gated rows 12 and 16. Design-first per the established discipline: a human-reviewed design brief (repurpose `PIPEWRIGHT_REDESIGN_IMPL_BRIEF.md`) precedes any code.

**Row 11 ships in ordered sub-PRs; this cycle opens PR-A only:**

- **PR-A (this cycle) — extract bootstrap detection into rules-as-data, behavior-preserving.** Move the `bootstrap.py` `_collect_candidates` if/elif chain into a declarative ruleset + a pure evaluator. **No new behavior:** prove byte-for-byte parity of the emitted bootstrap candidates against pre-refactor characterization snapshots (the Phase-2 golden discipline). Touches `backend/memory/bootstrap.py` + a new rules module + tests/docs only — no schema, no injection path, no gate/scope/Git contact.
- **PR-B (later) — emit repo reality signals advisorily.** Compute the repo signal *values* for the six dimensions `check_fact_against_signal` already supports (`db_engine`, `backend_framework`, `frontend_framework`, `test_runner`, `migration_tool`, `package_manager`); surface advisory-only. Mutates nothing; never marks stale / archives / auto-bumps `last_verified_at` (that is D8/B3, not this slice).
- **PR-C (later) — backfill test-command detection** (§23 order-row 1's detector) onto the shared ruleset.

**Hard stop after Row 11.** Run a real self-use smoke before starting any of: the backfill set above; the deeper Area B memory rows **12** (request-aware selection + mandatory tier, D5), **16** (post-run hygiene, D7), **19** (retriever + FTS rung 1), **23** (vector rung 2, D6); or the §21 thread UI rows (**22b–22e**, D13). The obsolete original hard-stop language ("before starting the stage driver, execution steering, full thread UI, or vector memory") is void for the engine rows — they shipped — and survives only as "before vector memory / thread UI."

## E.2 Accepted defaults for this cycle

**Row 11 (detection rules-as-data) is decision-free** — §24's gating map lists it among the items that "proceed on no open decisions," and §23 shows its only hard dependency is none (item 1's test-command detector is a *consumer*, backfilled in PR-C, not a prerequisite). No new default needs accepting to start this cycle.

Recorded for continuity (unchanged): §24's recommended defaults were accepted as written (single-sourced in §24 per E.5) — D5, D9, D10, D11, D12, D14 (chunk isolation advisory-only; hard gate rejected) — and **D2 = 1** (one INFRA_ERROR auto-retry) is already shipped in Phase 1. Those defaults gate the deeper memory rows (12/16/23) and the backfill set, **not** this cycle.

Vector memory (row 23), the retriever/FTS rung (row 19), request-aware selection (row 12), post-run hygiene (row 16), and the thread UI (rows 22b–22e) remain outside this window unless a later maintainer decision explicitly opens them. (Row 11, previously listed here, is now the active window above.)
